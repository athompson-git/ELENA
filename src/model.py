...
# B-L potential
class model_bl(generic_potential.generic_potential):

    def init(self, gBL=1.0, vev=1.0, xstep=0.00005, Tstep=0.00005):
        """
          Setting up the parameters
        """
    ...
    def V0(self, X):
        ...
        X = np.asanyarray(X)
        v = X[...,0]
        r = (3 / 4 / np.pi**2) * np.power(self.gBL*v, 4) * (np.log(np.power(v/self.vev, 2)) - 0.5)
    ...
    def Vtot(self, X, T, include_radiation=True):
        ...
        v = X[...,0]
        prefactor = 3 / 4 / np.pi**2
        y = prefactor * np.power(self.gBL*v, 4) * (np.log(np.power(v/self.vev, 2)) - 0.5) + np.power(self.gBL * T * v, 2)
