...
import numpy as np
from numpy import sqrt, pi, log, power
...
class GravitationalWave:
    ...
    def hstar_param(self):
        return 1.65e-5 * (self.Tstar/100.0) * power((self.gstar_D + g_rho_spline(self.Tstar))/100, 1/6)
    ...
    def sw_col(self, f):
        ...
        hstar = 1.65e-5 * (self.Tstar/100.0) * power((self.gstar_D + g_rho_spline(self.Tstar))/100, 1/6)
    ...
    def omega_sw(self, f):
        ...
        kappa = self.kappa()
        alpha = self.alpha
        return 8.5e-6 * self.shock_formation_scale() * self.sw_sw(f) * power(100/g_rho_spline(self.Tstar), 1/3) \
    ...
    def omega_turb(self, f):
        ...
        return 3.35e-4 * (1 - self.shock_formation_scale()) * self.vw * (1/self.betaByHstar) \
            * power(100/(self.gstar_D + g_rho_spline(self.Tstar)), 1/3) \
    ...
    def omega_col(self, f):
        ...
        return 1.67e-5 * power(1/self.betaByHstar, 2) * power(100/(self.gstar_D + g_rho_spline(self.Tstar)), 1/3) \
